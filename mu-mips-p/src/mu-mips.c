	...
	if(MEM_WB.opcode == 0x00)
		switch(MEM_WB.funct){
			...
			case 0x18: //MULT
				...
				break;
			case 0x19: //MULTU
				NEXT_STATE.HI = (uint32_t) MEM_WB.HI;
				NEXT_STATE.LO = (uint32_t) MEM_WB.LO;
				print_instruction(CURRENT_STATE.PC);
				break;
			case 0x1A: //DIV 
				NEXT_STATE.HI = MEM_WB.HI;
	...
	if(opcode == 0x00){
		switch(function){
			...
			case 0x03: //SRA 
				if ((rt & 0x80000000) == 1)
				{
					EX_MEM.ALUOutput =  ~(~CURRENT_STATE.REGS[rt] >> sa );
				}
				else{
					EX_MEM.ALUOutput = CURRENT_STATE.REGS[rt] >> sa;
				}
			...
			case 0x1A: //DIV  -???
				if(rt != 0)
				{
					EX_MEM.LO = (int32_t)rs / (int32_t)rt;
					EX_MEM.HI = (int32_t)rs % (int32_t)rt;
				}
				print_instruction(ID_EX.PC);
				break;
			case 0x1B: //DIVU -????
